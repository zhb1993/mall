...
<view class="container">
  ...
  <view class="sku" @tap="showSku">
    <view class="sku-tit">Seleccionado</view>
  ...
  <view class="cmt-wrap">
    <view class="cmt-tit" @tap="showComment">
      <view class="cmt-t">
        Comentarios
        <text class="cmt-good">Excelente{{prodCommData.positiveRating}}%</text>
      </view>
      <view class="cmt-count">
        Total de {{prodCommData.number}}Productos
        <text class="cmt-more"></text>
      </view>
    </view>
    <view class="cmt-cont">
      <view class="cmt-tag" @tap="showComment">
        <text>Todo({{prodCommData.number}})</text>
        <text>Excelente({{prodCommData.praiseNumber}})</text>
        <text>Moderado({{prodCommData.secondaryNumber}})</text>
        <text>Muy malo({{prodCommData.negativeNumber}})</text>
        <text>Foto({{prodCommData.picNumber}})</text>
      </view>
      ...
      <view class="cmt-more-v" v-if="prodCommPage.records.length > 2">
        <text @tap="showComment">Todos los comentarios</text>
      </view>
    </view>
  </view>
  <!-- 商品详情 -->
    ...
    <view class="pup-sku-main">
      <view class="pup-sku-header">
        ...
        <view class="pup-sku-prop">
          <text>No ha seleccionado</text> {{selectedProp.length>0?selectedProp+'，':''}}{{prodNum}}pieza
        </view>
      ...
      <view class="pup-sku-body">
        ...
        <view class="pup-sku-count">
          ...
          <view class="count-name">Cantidad</view>
        </view>
      </view>
      <view class="pup-sku-footer">
        <view class="btn cart" @tap="addToCart">Colección </view>
        <view class="btn buy" @tap="buyNow">Compre ahora</view>
      </view>
    </view>
  </view>
  <!-- 评价弹窗 -->
  <view class="cmt-popup" v-if="commentShow">
    ...
    <view class="cmt-cont">
      <view class="cmt-tag">
        <text @tap="getProdCommPage" data-evaluate="-1" :class="evaluate==-1?'selected':''">Todo({{prodCommData.number}})</text>
        <text @tap="getProdCommPage" data-evaluate="0" :class="evaluate==0?'selected':''">Excelente({{prodCommData.praiseNumber}})</text>
        <text @tap="getProdCommPage" data-evaluate="1" :class="evaluate==1?'selected':''">Moderado({{prodCommData.secondaryNumber}})</text>
        <text @tap="getProdCommPage" data-evaluate="2" :class="evaluate==2?'selected':''">Muy malo({{prodCommData.negativeNumber}})</text>
        <text @tap="getProdCommPage" data-evaluate="3" :class="evaluate==3?'selected':''">Foto({{prodCommData.picNumber}})</text>
      </view>
      <view class="cmt-items">
        <block v-if="prodCommPage.records.length">
          <view v-for="(item, index) in prodCommPage.records" :key="index" class="cmt-item">
            ...
            <view class="cmt-reply" v-if="item.replyContent">
              <text class="reply-tit">Responder：</text> {{item.replyContent}}
            </view>
          </view>
        </block>
        <view v-if="!prodCommPage.records.length" class="empty">Ninguno</view>
      </view>
      <view class="load-more" v-if="prodCommPage.pages > prodCommPage.current">
        <text @tap="getMoreCommPage">Cargando</text>
      </view>
    </view>
  </view>
</view>
