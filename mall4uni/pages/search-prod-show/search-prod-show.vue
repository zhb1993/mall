...
<view class="container">
  ...
  <view class="fixed-box">
    <view class="search-bar">
      <view class="search-box">
        <input placeholder="Buscar" class="sear-input" :value="prodName" @input="getSearchContent" confirm-type="search" @confirm="toSearchConfirm"></input>
    ...
    <view class="tabs">
      <text :class="'tab-item complete ' + (sts==0?'on':'')" @tap="onStsTap" data-sts="0">Productos varios </text>
      <text :class="'tab-item ' + (sts==1?'on':'')" @tap="onStsTap" data-sts="1">Ventas</text>
      <text :class="'tab-item ' + (sts==2?'on':'')" @tap="onStsTap" data-sts="2">Precio</text>
    </view>
  </view>
      ...
      <block v-for="(item, index) in searchProdList" :key="index">
        <view class="show-item" @tap="toProdPage" :data-prodid="item.prodId">
          ...
          <view class="prod-text-right">
            ...
            <view class="cate-prod-info">{{item.praiseNumber}}Comentarios {{item.positiveRating}}%Evaluar</view>
  ...
  <view v-if="!searchProdList.length" :class="['empty',showType==1? 'empty-top':'']">Ninguno</view>
</view>
</template>
